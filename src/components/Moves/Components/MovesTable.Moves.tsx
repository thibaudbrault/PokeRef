...
function MovesTable({ moves, toggleState }: Props) {
	...
	useEffect(() => setFilteredMoves(
		moves.filter((moves: { name: string; }) => moves.name
			.replace(/-/g, ' ')
			.toLowerCase()
								...
								<TRow key={m.id}>
									...
									<TEffect>
										{m?.flavor_text_entries?.map(
											(mf: { language: { name: string; }; flavor_text: string | number | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | React.ReactFragment | null | undefined; }) =>
												mf.language.name === 'en' &&
